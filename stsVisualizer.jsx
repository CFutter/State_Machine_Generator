...
const E = [
  ...
  // ── SLIMES ──
  {id:"acid_slime_l",name:"Acid Slime (L)",emoji:"🟢",type:"normal",desc:"3-move AI + splits at 50% → 2× Acid Slime M.",hp:a=>a>=7?[68,72]:[65,69],
  ...
  {id:"acid_slime_m",name:"Acid Slime (M)",emoji:"🟩",type:"normal",desc:"3-move AI: corrosive, tackle, weak.",hp:a=>a>=7?[29,34]:[28,32],
    ...
    notes:["Can't same move 3× (2× for some)","Complex probability fallback"],asc:[{level:2,text:"Corrosive 7→8, Tackle 10→12"},{level:7,text:"HP 28-32→29-34"},{level:17,text:"Stricter"}]},
  {id:"acid_slime_s",name:"Acid Slime (S)",emoji:"💚",type:"normal",desc:"Alternates attack and Weak.",hp:a=>a>=7?[9,13]:[8,12],
    ...
    notes:a=>a>=17?["A17: always Tackle first, alternates"]:["50/50 first move, then alternates"],asc:[{level:2,text:"Tackle 3→4"},{level:7,text:"HP 8-12→9-13"},{level:17,text:"Always starts Tackle"}]},
  {id:"spike_slime_l",name:"Spike Slime (L)",emoji:"🔴",type:"normal",desc:"Tackle + Slimed + Frail. Splits at 50%.",hp:a=>a>=7?[67,73]:[64,70],
    ...
    notes:["Splits → 2 Spike M at ≤50% HP","Has Split power"],asc:[{level:2,text:"Tackle 16→18"},{level:7,text:"HP 64-70→67-73"},{level:17,text:"Frail 2→3, Lick can't 2×"}]},
  {id:"spike_slime_m",name:"Spike Slime (M)",emoji:"🟥",type:"normal",desc:"Tackle + Slimed and applies Frail.",hp:a=>a>=7?[29,34]:[28,32],
    ...
    notes:a=>a>=17?["Can't Lick 2×"]:["Can't same 3×"],asc:[{level:2,text:"Tackle 8→10"},{level:7,text:"HP 28-32→29-34"},{level:17,text:"Lick can't 2×"}]},
  ...
  {id:"looter",name:"Looter",emoji:"💰",type:"normal",desc:"Steals gold, then flees. Kill fast!",hp:a=>a>=7?[46,50]:[44,48],
    ...
    notes:a=>[`Steals ${a>=17?20:15} gold per Mug/Lunge hit`,"Mug×2 → 50/50 Lunge or Smoke Bomb → Escape","Gold returned on kill"],
  ...
  {id:"blue_slaver",name:"Blue Slaver",emoji:"⛓",type:"normal",desc:"Stabs and rakes with Weak.",hp:a=>a>=7?[48,52]:[46,50],
    ...
    notes:a=>a>=17?["Can't Rake 2× in a row","Can't Stab 3×"]:["Can't Stab 3×","Can't Rake 3×"],
    asc:[{level:2,text:"Stab 12→13, Rake 7→8"},{level:7,text:"HP 46-50→48-52"},{level:17,text:"Rake Weak 1→2, can't Rake 2×"}]},
  {id:"red_slaver",name:"Red Slaver",emoji:"🔗",type:"normal",desc:"Stabs, scrapes with Vuln, Entangles once.",hp:a=>a>=7?[48,52]:[46,50],
    states:[{id:"stab",name:"Stab",type:"attack",detail:a=>a>=2?"14 dmg":"13 dmg"},{id:"scrape",name:"Scrape",type:"attack_debuff",detail:a=>a>=17?`${a>=2?9:8} +Vuln2`:`${a>=2?9:8} +Vuln1`},{id:"entangle",name:"Entangle",type:"debuff",detail:()=>"Can't play Atks"}],
    positions:{_start:{x:70,y:90},stab:{x:300,y:90},scrape:{x:560,y:90},entangle:{x:420,y:370}},
    transitions:a=>[{from:null,to:"stab",prob:1,label:"Turn 1"},{from:"stab",to:"stab",prob:.35,label:"~35%"},{from:"stab",to:"scrape",prob:.4,label:"~40%"},{from:"stab",to:"entangle",prob:.25,label:"25%*"},{from:"scrape",to:"stab",prob:.5,label:"~50%"},{from:"scrape",to:"scrape",prob:.5,label:"~50%"},{from:"entangle",to:"stab",prob:.45,label:"~45%"},{from:"entangle",to:"scrape",prob:.55,label:"~55%"}],
    notes:a=>["Stab on turn 1 always","Entangle used ONCE only (25% chance when eligible)",a>=17?"Can't Scrape 2× in a row":"Can't Scrape 3× in a row"],
    asc:[{level:2,text:"Stab 13→14, Scrape 8→9"},{level:7,text:"HP 46-50→48-52"},{level:17,text:"Scrape Vuln 1→2, can't 2×"}]},
  // ═══ ELITES ═══
  {id:"gremlin_nob",name:"Gremlin Nob",emoji:"👹",type:"elite",desc:"Enrages on Skills. Punishes defense.",hp:a=>a>=8?[85,90]:[82,86],
    ...
    notes:a=>a>=18?["A18: Bash forced if not in last 2","Can't Rush 3×"]:["Can't same 3×","STR on each Skill"],asc:[{level:3,text:"Rush 14→16, Bash 6→8"},{level:8,text:"HP 82-86→85-90"},{level:18,text:"Enrage +3, Bash deterministic"}]},
  {id:"lagavulin",name:"Lagavulin",emoji:"😴",type:"elite",desc:"Sleeps with 8 Metallicize.",hp:a=>a>=7?[109,113]:[104,110],
    ...
    transitions:()=>[{from:null,to:"sleeping",prob:1,label:"Start"},{from:"sleeping",to:"sleeping",prob:null,label:"≤3 turns"},{from:"sleeping",to:"attack",prob:null,label:"Wakes"},{from:"attack",to:"siphon",prob:1,label:"Always"},{from:"siphon",to:"attack",prob:1,label:"Always"}],
    notes:["Wakes after 3 turns or damage","8 Metallicize while asleep"],asc:[{level:2,text:"Attack 18→20"},{level:7,text:"HP 104-110→109-113"},{level:17,text:"Siphon -1→-2"}]},
  {id:"sentry",name:"Sentry",emoji:"🔷",type:"elite",desc:"Alternates Bolt (Dazed) and Beam. 3 in encounter.",hp:a=>a>=8?[39,45]:[38,42],
    ...
    notes:["3 Sentries in encounter","Even-indexed starts Bolt, odd starts Beam","Each has Artifact 1","Strictly alternates"],asc:[{level:3,text:"Beam 9→10"},{level:8,text:"HP 38-42→39-45"},{level:18,text:"Dazed 2→3"}]},
  // ═══ BOSSES ═══
  {id:"guardian",name:"The Guardian",emoji:"⚙",type:"boss",desc:"Mode Shift boss: offensive cycle, curls up on damage threshold.",hp:a=>a>=9?[250,250]:[240,240],
    states:[{id:"charge_up",name:"Charge Up",type:"defend",detail:()=>"9 block"},{id:"fierce_bash",name:"Fierce Bash",type:"attack",detail:a=>a>=4?"36 dmg":"32 dmg"},{id:"vent_steam",name:"Vent Steam",type:"debuff",detail:()=>"Weak 2 +Vuln 2"},{id:"whirlwind",name:"Whirlwind",type:"attack",detail:()=>"5 dmg ×4"},{id:"close_up",name:"Close Up",type:"buff",detail:a=>a>=19?"Sharp Hide 4":"Sharp Hide 3"},{id:"roll",name:"Roll Attack",type:"attack",detail:a=>a>=4?"10 dmg":"9 dmg"},{id:"twin_slam",name:"Twin Slam",type:"mixed",detail:()=>"8 dmg ×2"}],
    ...
    transitions:()=>[{from:null,to:"charge_up",prob:1,label:"Turn 1 (Open)"},{from:"charge_up",to:"fierce_bash",prob:1,label:"Next"},{from:"fierce_bash",to:"vent_steam",prob:1,label:"Next"},{from:"vent_steam",to:"whirlwind",prob:1,label:"Next"},{from:"whirlwind",to:"charge_up",prob:1,label:"Cycle"},{from:"charge_up",to:"close_up",prob:null,label:"Mode Shift!"},{from:"fierce_bash",to:"close_up",prob:null,label:"Mode Shift!"},{from:"vent_steam",to:"close_up",prob:null,label:"Mode Shift!"},{from:"whirlwind",to:"close_up",prob:null,label:"Mode Shift!"},{from:"close_up",to:"roll",prob:1,label:"Defensive"},{from:"roll",to:"twin_slam",prob:1,label:"Next"},{from:"twin_slam",to:"whirlwind",prob:1,label:"Opens up"}],
    notes:a=>[`Mode Shift: curls up after ${a>=19?40:a>=9?35:30} dmg (+10 each time)`,"Gains 20 block on curl-up, Sharp Hide (thorns)","Twin Slam removes Sharp Hide and reopens","Offensive: Charge→Bash→Vent→Whirl cycle"],
    asc:[{level:4,text:"Fierce Bash 32→36, Roll 9→10"},{level:9,text:"HP 240→250, Threshold 30→35"},{level:19,text:"Threshold→40, Sharp Hide 3→4"}]},
  {id:"hexaghost",name:"Hexaghost",emoji:"👻",type:"boss",desc:"Fixed 7-turn cycle. Inferno upgrades Burns.",hp:a=>a>=9?[264,264]:[250,250],
    ...
    transitions:()=>[{from:null,to:"activate",prob:1,label:"Turn 1"},{from:"activate",to:"sear",prob:1,label:"T2"},{from:"sear",to:"tackle",prob:1,label:"T3"},{from:"tackle",to:"sear",prob:null,label:"T4"},{from:"sear",to:"inflame",prob:null,label:"T5"},{from:"inflame",to:"tackle",prob:null,label:"T6"},{from:"tackle",to:"sear",prob:null,label:"T7"},{from:"sear",to:"inferno",prob:null,label:"All orbs lit"},{from:"inferno",to:"sear",prob:1,label:"Cycle reset"}],
    notes:["Fixed cycle: Activate→Sear→Tackle→Sear→Inflame→Tackle→Sear→Inferno","Inferno upgrades all Burns in deck","Divider = (player HP / 12 + 1) × 6 hits"],
    asc:[{level:4,text:"Tackle 5→6×2, Inferno 2→3×6"},{level:9,text:"HP 250→264"},{level:19,text:"Sear 1→2 Burn, Inflame STR 2→3"}]},
  {id:"slime_boss",name:"Slime Boss",emoji:"🟤",type:"boss",desc:"Sticky→Prep→Slam cycle. Splits at 50%.",hp:a=>a>=9?[150,150]:[140,140],
    ...
    transitions:()=>[{from:null,to:"goop",prob:1,label:"Turn 1"},{from:"goop",to:"prep",prob:1,label:"Next"},{from:"prep",to:"slam",prob:1,label:"Next"},{from:"slam",to:"goop",prob:1,label:"Cycle"},{from:"goop",to:"split",prob:null,label:"HP≤50%"},{from:"prep",to:"split",prob:null,label:"HP≤50%"},{from:"slam",to:"split",prob:null,label:"HP≤50%"}],
    notes:["Splits → Spike Slime L + Acid Slime L at ≤50%","Children inherit current HP","Fixed cycle: Goop→Prep→Slam→repeat","Has Split power"],
...
// ═══ SVG HELPERS ═══
function cEdge(f,t,cv=.35){const dx=t.x-f.x,dy=t.y-f.y,d=Math.sqrt(dx*dx+dy*dy);if(!d)return null;const a=Math.atan2(dy,dx),nr=f.r||NR+4,nT=t.r||NR+4,nx=-dy/d,ny=dx/d,o=cv*55;const sx=f.x+nr*Math.cos(a+cv*.18),sy=f.y+nr*Math.sin(a+cv*.18),ex=t.x-nT*Math.cos(a-cv*.18),ey=t.y-nT*Math.sin(a-cv*.18),mx=(sx+ex)/2+nx*o,my=(sy+ey)/2+ny*o;return{path:`M ${sx} ${sy} Q ${mx} ${my} ${ex} ${ey}`,lx:(sx+2*mx+ex)/4,ly:(sy+2*my+ey)/4,ex,ey,ea:Math.atan2(ey-my,ex-mx)};}
function sLoop(n,d="top"){const cx=n.x,cy=n.y,r=NR+4;if(d==="top")return{path:`M ${cx-12} ${cy-r} C ${cx-50} ${cy-r-65},${cx+50} ${cy-r-65},${cx+12} ${cy-r}`,lx:cx,ly:cy-r-50,ex:cx+12,ey:cy-r,ea:.45};if(d==="right")return{path:`M ${cx+r} ${cy-12} C ${cx+r+65} ${cy-50},${cx+r+65} ${cy+50},${cx+r} ${cy+12}`,lx:cx+r+50,ly:cy,ex:cx+r,ey:cy+12,ea:Math.PI*.65};return{path:`M ${cx+12} ${cy+r} C ${cx+50} ${cy+r+65},${cx-50} ${cy+r+65},${cx-12} ${cy+r}`,lx:cx,ly:cy+r+50,ex:cx-12,ey:cy+r,ea:Math.PI+.45};}
...
function Diagram({enemy,asc,hov,setHov}){const pos=enemy.positions,trans=typeof enemy.transitions==="function"?enemy.transitions(asc):enemy.transitions;
  ...
  const edges=trans.map(t=>{const fp=t.from?pos[t.from]:pos._start,tp=pos[t.to];if(!fp||!tp)return null;
    if(t.from&&t.from===t.to){const dir=tp.y<=180?"top":tp.x>=520?"right":"bottom";const l=sLoop(tp,dir);return{...l,label:t.label,fId:t.from,tId:t.to,dash:t.prob===null};}
    const rev=t.from&&pairs.has(`${t.to}->${t.from}`);const ed=cEdge({...fp,r:t.from?NR+4:15},{...tp,r:NR+4},rev?.5:.3);
...
export default function App(){
  const [selId,setSelId]=useState("cultist"),[asc,setAsc]=useState(0),[hov,setHov]=useState(null),[search,setSearch]=useState(""),[fType,setFType]=useState("all"),[showEnc,setShowEnc]=useState(false);
  const enemy=E.find(e=>e.id===selId);const hp=enemy.hp(asc);const notes=typeof enemy.notes==="function"?enemy.notes(asc):enemy.notes;
  ...
  const filtered=E.filter(e=>(fType==="all"||e.type===fType)&&(!search||e.name.toLowerCase().includes(search.toLowerCase())));
  const grouped={normal:[],elite:[],boss:[]};filtered.forEach(e=>{if(grouped[e.type])grouped[e.type].push(e);});
  const sel=id=>{setSelId(id);setHov(null);};
        ...
        {!showEnc?(<><div style={{padding:"8px 8px 5px"}}><input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." style={{width:"100%",boxSizing:"border-box",padding:"5px 7px",borderRadius:"4px",border:`1px solid ${C.border}`,background:C.bg,color:C.text,fontSize:"10.5px",fontFamily:"'Fira Code',monospace",outline:"none"}}/>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"7px"}}><div style={{fontSize:"7.5px",color:C.textMuted,letterSpacing:"1.5px",padding:"2px 0 5px",textTransform:"uppercase"}}>Act 1 Encounters</div>
            {ENC.map((enc,i)=><div key={i} style={{padding:"6px 8px",marginBottom:"3px",borderRadius:"5px",border:`1px solid ${C.border}`,background:C.bg}}>
              ...
              <div style={{display:"flex",gap:"3px",marginTop:"3px",flexWrap:"wrap"}}>{enc.enemies.map(eid=>{const en=E.find(e=>e.id===eid);if(!en)return null;return<button key={eid} onClick={()=>{sel(eid);setShowEnc(false);}} style={{padding:"2px 6px",borderRadius:"3px",fontSize:"9px",border:`1px solid ${C.borderHi}`,background:"#181838",color:C.textDim,cursor:"pointer",fontFamily:"'Cinzel',serif"}}>{en.emoji} {en.name}</button>;})}</div></div>)}
          </div>)}
      </div>
